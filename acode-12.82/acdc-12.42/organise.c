/* organise.c (acdc) - copyright Mike Arnautov 1990-2020.
...
void mandate (char *item, int type, ...)
{
   ...
   if ((np = fndsymb(TESTSYMBOL, item)) == NULL)
   {
      ...
      if (type == OBJ || type == WORD || type == COMMA || type == SEMICOLON)
      {
...
/* Check for mandatory symbols and add them if missing */
   ...
   {
     mandate ("parser", VAR);
     if (parser_word [0] == 0) mandate ("and", COMMA);
     if (parser_word [1] == 0) mandate ("then", SEMICOLON);
     mandate ("blank", TEXT, " \n");
   }
   mandate ("inhand", LOC);
   mandate ("here", VAR);
 ...
 */
   ...
   else if ((np = fndsymb(TESTSYMBOL, "again")) != NULL)
   {
      if (np -> type != VERB)
