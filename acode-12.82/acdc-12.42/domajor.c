...
//         if (tp[index] && strcmp(tp[index], "utf8") == 0)
//            utf8 = 1;
         break;
 ...
 */
      ...
      case COMMA:
      case SEMICOLON:
         gripe (tp [0], "Should only be used as a PARSER specification.");
         break;
      ...
      case PARSER:
         index = 1;
         {
            char *ptype [] = {"comma", "semicolon", "repeater", "noise"};
            int ptp = -1;
            int i;
            for (i = 0; i < 4; i++)
            {
               if (strcmp (ptype [i], tp [1]) == 0)
               {
                   ptp = i;
                   parser_word [i] = 1;
                   break;
            ...
            if (ptp < 0)
               gripe (tp[1], "Unknown parser word!");
            if (*ptype [ptp] != 'r')
            {
               while (tp [++index] != NULL)
                  storword (tp [index], ptp + PARSER + 1, next_vocaddr);
            }
            else
            {
              repeater = getnames (PARSER, NULL);
              repeater -> refno = type_counts[VERB]++;
              repeater -> used_count = 1;   // Don't report as not used!
            }
         }
         break;
