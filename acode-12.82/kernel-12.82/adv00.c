 ...
 */
#define KERNEL_VERSION "12.82, 19 Apr 2021"
/*
 * 05 May 21   MLA        Added test for ndeedcmd.
 ...
 *
 * int utf8_recase(int casing, char *string, char *recased, int room)
 *
 ...
 *   room     gives the length of the buffer pointed at by 'recased'
 *
...
#else /* UTF8 == 0 -- define a fake utf8_recase() */

int utf8_recase(int casing, char *string, int len, char *recased, int room)
{
  while (len && *string)
  {
     *recased++ = casing == 'L' ? tolower(*string) : toupper(*string);
     string++;
     len--;
     if (casing == 'C')
       casing = 'L';
  }
  return strlen(recased);
}
